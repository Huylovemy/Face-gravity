if getgenv().isloading then
    warn("[Bear]: Already loading, wait!!!!")
    return
end
...
print("")
print("BearHub | Anything Below Here Is Probably BearHub Related Errors!")

if not isfolder("Bearhub") then makefolder("Bearhub") end
if not isfolder("Bearhub/themes") then makefolder("Bearhub/themes") end
if not isfile("Bearhub/themes/default.txt") then writefile("Bearhub/themes/default.txt", "BearIsh") end
if not isfile("Bearhub/themes/BearIsh.json") then writefile("Bearhub/themes/BearIsh.json", '{"MainColor":"131218","FontFace":"Code","AccentColor":"a970ff","OutlineColor":"262434","BackgroundColor":"0b0b0d","FontColor":"e8e6f2"}') end

loadstring(game:HttpGet("https://apigetBear.vercel.app/Modules/v2/Inv.lua",true))()

if not isfile("UsedOneTime.Bear") then
    writefile("UsedOneTime.Bear","1")
elseif not isfile("AlreadyRated.Bear") then
    loadstring(game:HttpGet(
    "https://raw.githubusercontent.com/not-gato/Bear/refs/heads/main/Modules/v2/Rating.lua"
    ))()
    writefile("AlreadyRated.Bear","1")
end

if getgenv().Bearshared and getgenv().Bearshared.isloaded == true then
    warn("BearHub is already loaded. Skipping initialization.")
    return
end
...
local Config = {
    ...
    Assets = {
        ...
        Checkmark = "rbxassetid://134852113716171",
...
LogoImage.ZIndex = 1000134852113716171
...
function Controller:Fail()
    ...
    Controller:SetStatus("Failed To Start BearHub :(")
end
...
Controller:Start()
s("BearHub Loader v2.4.2 initialized")
s("Checking API Status...")
loadstring(game:HttpGet("https://raw.githubusercontent.com/not-gato/Near/refs/heads/main/Modules/v2/API.lua",true))()
task.wait(0.1)

s("Creating global variables...")
task.wait(0.05)
getgenv().Bearshared={version="2.4.2",gamename=MarketplaceService:GetProductInfo(game.PlaceId).Name,issupported=false,playername=LocalPlayer.Name,playerid=LocalPlayer.UserId,isloaded=false,devnote="Bear Hud 💖 by Quang Huy"}
s("Player: "..LocalPlayer.Name.." (ID: "..LocalPlayer.UserId..")")
task.wait(0.05)
s("Game: "..getgenv().Bearshared.gamename)
task.wait(0.14)
s("Checking game compatibility...")
task.wait(0.02)
local bc=game.PlaceId
s("Game ID: "..tostring(bc))
s("Developer Note: "..getgenv().Bearshared.devnote)
s("BearHub is a non-profit, source avaliable script hub. Any form of help, such as donating, supporting, or joining the Discord server, encourages me to continue this project, thanks! :D")
bd={[12240122896]="https://apigetBear.vercel.app/Games/FigureL.lua",[136801880565837]="https://apigetBear.vercel.app/Games/Flick.lua",[893973440]="https://github.com/not-gato/Bear/raw/refs/heads/main/Games/Flee.lua",[132745842491660]="https://github.com/not-gato/Bear/raw/refs/heads/main/Games/Flee.lua",[106344247300586]="https://github.com/not-gato/Bear/raw/refs/heads/main/Games/Flee.lua",[125624013879756]="https://github.com/not-gato/Bear/raw/refs/heads/main/Games/Flee.lua"}
local be,bf

if bd[bc]then
    getgenv().Bearshared.issupported=true
    s("Game verified: "..getgenv().Bearshared.gamename)
    task.wait(0.05)
    ...
    getgenv().Bearshared.issupported=false
    s("Game not supported, loading universal")
    task.wait(0.03)
    s("Fetching universal script...")
    task.wait(0.1)
    be,bf=pcall(function()
        local scriptUrl="https://raw.githubusercontent.com/not-gato/Bear/refs/heads/main/Games/Universal.lua"
        local src=game:HttpGet(scriptUrl)
        local func,err=loadstring(src,scriptUrl)
        if not func then error(err) end
        func()
...
if be then
    ...
    loadstring(game:HttpGet("https://raw.githubusercontent.com/not-gato/Bear/refs/heads/main/Modules/v2/Log.lua"))()
    getgenv().Bearshared.isloaded=true
    ...
    getgenv().Bearshared.isloaded = false
    Controller:Fail()
    
    local errorMsg=tostring(bf):gsub("`","")
    local kickTitle="BearHub"
    local kickBody=
    "<font color='rgb(255,100,100)'>An error occurred and BearHub must close.</font>\n\n"..
    ...
    if success and cKickModule and cKickModule.cKick then
        ...
        game.Players.LocalPlayer:Kick("Custom kick module failed to load! wtf?!??, Press F9 To see BearHub Error!")
        warn("[Bear]: BearHub Loader Error:".. bf)
    end
end
